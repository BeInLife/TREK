 ...
 * PluginHostDepsFactory). Exports only the legacy services/exchangeRateService
 * names still consumed outside the container (getRates — the dead
 * convertWithRates export was dropped in the post-fold quirk fixes), so
 * repointing a consumer is an import-path-only diff. Inside the container,
 * inject ExchangeRatesService instead.
